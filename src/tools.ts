...
import { DesignOrchestrator } from "./design/orchestrator.js";
...
export const tools = [
  ...
  {
    name: "generate_design_artifacts",
    description: "Generate design artifacts via provider-agnostic orchestrator.",
    inputSchema: {
      type: "object",
      properties: {
        ...monetizationContextSchema,
        ...
        task_type: {
          type: "string",
          enum: ["screen", "component", "tokens", "design_system_update"]
        },
        intent: { type: "string" },
        repo_root: { type: "string" },
        target_paths: { type: "array", items: { type: "string" } },
        constraints: { type: "array", items: { type: "string" } },
        design_contract_path: { type: "string" },
        design_contract_content: { type: "string" },
        prefer_provider: { type: "string" },
        mode: { type: "string", enum: ["draft", "handoff"] },
        variants: { type: "number" },
        output_formats: { type: "array", items: { type: "string" } }
      },
      required: ["project_id", "task_id", "task_type", "intent"]
    }
  }
];
...
export const createToolRouter = (engine: WorkflowEngine, sqliteStore?: SqliteStore) => {
  ...
  const designOrchestrator = new DesignOrchestrator({
    primary_by_task_type: {
      ...
      component: "stitch",
      tokens: "figma",
      design_system_update: "figma"
    },
    fallback_order: ["stitch", "figma"]
  });
  designOrchestrator.registerProvider(new StitchProvider());
  designOrchestrator.registerProvider(new FigmaProvider());
  ...
  const handleToolCall = (name: string, args: Record<string, unknown>) => {
    return applyMonetization(
      ...
      switch (name) {
        ...
        case "generate_design_artifacts": {
          track("generate_design_artifacts", {
            ...basePayload,
            project_id: args.project_id,
            task_id: args.task_id,
            task_type: args.task_type
          });
          const projectTask = engine.getProjectTask(
            args.project_id as string,
            args.task_id as string
          );
          if (!projectTask) {
          ...
          return designOrchestrator.run(request);
        }
        default:
