...
const resources = [
  ...
  {
    uri: "buildrail://design/spec",
    name: "Design contract spec",
    mimeType: "application/json",
    description: "Canonical design contract loaded from DESIGN.md."
  }
];
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  ...
  if (uri === "buildrail://design/spec") {
    const designPath = resolve(process.cwd(), "DESIGN.md");
    const exists = existsSync(designPath);
    const payload = exists
      ...
      : {
          ...
          path: designPath,
          content: null,
          message: "DESIGN.md not found in repository root."
        };
    return {
      contents: [
        {
